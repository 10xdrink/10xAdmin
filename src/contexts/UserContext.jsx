...
export const UserProvider = ({ children }) => {
  ...
  const inviteUser = useCallback(async (email, role) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const errorMsg = typeof data.message === 'string' ? data.message : 'Failed to invite user.';
        setError(errorMsg);
        toast.error(errorMsg);
      }
    } catch (err) {
      console.error('inviteUser error:', err); // Debugging
      // Ensure we get a string message from the error
      const errorMessage = err?.message || String(err) || 'Failed to invite user.';
      setError(errorMessage);
      toast.error(errorMessage);
      throw new Error(errorMessage); // Re-throw with string message
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const getTotalUserCount = useCallback(async () => {
    ...
    try {
      ...
      console.log('getTotalUserCount raw response:', count); // Debugging
      if (typeof count !== 'number') {
        console.warn('getTotalUserCount: Unexpected response format:', count);
        return 0; // Fallback
      }
      return count; 
    } catch (err) {
      console.error('getTotalUserCount error details:', err); // Extended debugging
      setError(typeof err === 'string' ? err : 'Failed to fetch total user count');
      toast.error(`Failed to fetch total user count: ${err}`);
  ...
  const getUserMetrics = useCallback(async (id) => {
    ...
    try {
      ...
      console.log('getUserMetrics response in context:', data);
      
      if (data.success) {
        return data.metrics; // Return just the metrics object
      } else {
        setError(data.message || 'Failed to fetch user metrics.');
        toast.error(data.message || 'Failed to fetch user metrics.');
        // Return empty metrics with status
        return {
          shopping: {},
          engagement: {},
          recency: {},
          ...
          _status: {
            error: true,
            message: data.message || "Error fetching metrics"
          }
        };
      }
    } catch (err) {
      console.error('getUserMetrics error:', err); // Debugging
      setError(err.message || 'An unexpected error occurred fetching metrics.');
      toast.error(err.message || 'An unexpected error occurred fetching metrics.');
      return {
        shopping: {},
        engagement: {},
        recency: {},
  ...
  const getNewUsersCount = useCallback(async (days = 30) => {
    try {
      ...
      console.log('New Users Count raw response:', count); // Debugging
      if (typeof count !== 'number') {
        console.warn('getNewUsersCount: Unexpected response format:', count);
        return 0; // Fallback
      }
      return count;
    } catch (err) {
      console.error('getNewUsersCount error details:', err); // Extended debugging
  ...
  const getReturningUsersCount = useCallback(async () => {
    try {
      ...
      console.log('Returning Users Count raw response:', count); // Debugging
      if (typeof count !== 'number') {
        console.warn('getReturningUsersCount: Unexpected response format:', count);
        return 0; // Fallback
      }
      return count;
    } catch (err) {
      console.error('getReturningUsersCount error details:', err); // Extended debugging
   ...
   * @returns {Object} - Object containing counts of active and inactive users
   */
  const getUserCountsByStatus = useCallback(async () => {
    try {
      const countData = await userService.getUserCountsByStatus();
      console.log('User Counts By Status:', countData);
      return countData;
    } catch (err) {
      console.error('getUserCountsByStatus error details:', err);
      toast.error(`Failed to fetch user status counts: ${err}`);
      return { activeUsers: 0, inactiveUsers: 0 }; // Return a fallback value
    }
  }, []);

  /**
   * Cancel a pending invitation
   * @param {string} email - Email address of the invitation to cancel
   * @returns {Promise<Object>} - Response from cancel invitation request
   */
  const cancelPendingInvitation = useCallback(async (email) => {
    setLoading(true);
    setError(null);
    try {
      const data = await userService.cancelInvitation(email);
      console.log('cancelPendingInvitation response:', data); // Debugging
      if (data.success) {
        toast.success('Invitation canceled successfully.');
        return data;
      } else {
        const errorMsg = typeof data.message === 'string' ? data.message : 'Failed to cancel invitation.';
        setError(errorMsg);
        toast.error(errorMsg);
        return null;
      }
    } catch (err) {
      console.error('cancelPendingInvitation error:', err);
      const errorMessage = err?.message || String(err) || 'Failed to cancel invitation.';
      setError(errorMessage);
      toast.error(errorMessage);
